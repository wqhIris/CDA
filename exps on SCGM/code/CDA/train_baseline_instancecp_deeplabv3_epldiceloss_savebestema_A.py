...
from config_scgm_deeplabv3_epldiceloss_A import default_config
...
def train(label_loader, unlabel_loader_0, unlabel_loader_1, test_loader, val_loader, learning_rate, weight_decay, num_epoch, model_path, niters_per_epoch):
    ...
    # ema_model = ModelEMA(model1, 0.7) #!!!0.7)#!!! 0.8) #!!!0.999)
    ...
    savedir='./tmodel_scgm/1gpu/baseline_instance_deeplabv3_epldiceloss_savebestema_counttime_A/'
    ...
    for epoch in range(num_epoch):
        ...
        for idx in pbar:
            ...
            #!!!l2l
            sup_imgs_l2l, mask_l2l, _, _ = CP(imgs, mask[:,0,:,:], imgs, mask[:,0,:,:])
            '''
            import torchvision
            ...
            # torchvision.utils.save_image(unsup_imgs_0,'unsup_imgs_0_forcp_{}.png'.format(epoch))
            # torchvision.utils.save_image(pseudo_labels.unsqueeze(1).float(),'pseudo_labels_forcp_{}.png'.format(epoch))
            ...
            volume_batch = torch.cat([imgs, sup_imgs_l2l], 0)
            label_batch = torch.cat([mask[:,0,:,:], mask_l2l], 0)
            
            outputs, _ = model1(volume_batch) #!!! shape:[b,2,288,288]
            outputs_soft = torch.softmax(outputs, dim=1)
            
            '''
            outputs_soft_l = torch.argmax(outputs_soft.detach(),dim=1,keepdim=False)
            torchvision.utils.save_image(outputs_soft_l[config['batch_size']*1:config['batch_size']*2].unsqueeze(1).float(),'outputs_soft_l_forcp_{}.png'.format(epoch))
            if epoch > 3:
                print(kiki)
            '''
            ...
            pseudo_supervision_aug1 = dice_loss(outputs_soft[config['batch_size']*1:config['batch_size']*2][:,0,:,:], label_batch[config['batch_size']*1:config['batch_size']*2])
            
            # if epoch <= 3:
            #     loss = labeled_loss_1 + pseudo_supervision_aug1
            # else:
            #     loss = labeled_loss_1 + pseudo_supervision + pseudo_supervision_aug1
            loss = labeled_loss_1 + pseudo_supervision_aug1
            epoch_loss.append(float(loss))
            epoch_loss_labeled.append(float(labeled_loss_1))
            # epoch_loss_pseudo.append(float(pseudo_supervision))
            epoch_loss_pseudo_aug.append(float(pseudo_supervision_aug1))
    ...
    print(
        "Total training time: {} ({:.4f}s / epoch)".format(total_training_str, total_training_time / num_epoch))
