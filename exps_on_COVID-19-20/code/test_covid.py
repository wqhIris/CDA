...
def pngs_2_niigz(args, png_results_path, nii_results_path, file_volume_name):
    volume_files =  pd.read_excel(args.dataroot_path + "data/{}/{}".format(args.dataset_name, file_volume_name)) #!!!args.root_path + "data/{}/{}".format(args.dataset_name, file_volume_name))
    length = volume_files.shape[0]
    for idx in range(length):
        volume_file = volume_files.iloc[idx][0]
        # load original nii
        ori_path= args.dataroot_path + "data/{}/NII/{}".format(args.dataset_name, volume_file+'_ct.nii.gz') #!!!args.root_path + "data/{}/NII/{}".format(args.dataset_name, volume_file+'_ct.nii.gz')
...
def evaluate_nii(args, nii_results_path, file_volume_name,tao_NSD=3):
    nsd_sum = 0
    dice_sum = 0
    hau_sum = 0
    volume_files =  pd.read_excel(args.dataroot_path + "data/{}/{}".format(args.dataset_name, file_volume_name)) #!!!args.root_path + "data/{}/{}".format(args.dataset_name, file_volume_name))
    ...
    for idx in range(length):
        volume_file = volume_files.iloc[idx][0]
        # load gt nii
        gt_path= args.dataroot_path + "data/{}/NII/{}".format(args.dataset_name, volume_file+'_seg.nii.gz') #!!!args.root_path + "data/{}/NII/{}".format(args.dataset_name, volume_file+'_seg.nii.gz')
        ...
        #tao_NSD: 1(lung) 3(infection)
        nsd = compute_surface_dice_at_tolerance(surface_distances, tao_NSD)        
...
def get_model_metric(args, model, snapshot_path, model_name, mode='test', post=False):
    model.eval()
    ...
    for batch_idx, (image, label, file_name, lung) in enumerate(val_dataloader):
        image = image.cuda()
        label = label.cuda()

        with torch.no_grad():
            out_main = model(image)
            out = torch.argmax(torch.softmax(out_main, dim=1), dim=1)
            # print('out.shape, lung.shape:',out.shape, lung.shape)
            if post:
                out[lung==0]=0
            out = out.squeeze(0)
