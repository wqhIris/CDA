...
parser.add_argument('--exp', type=str, default='baseline_tumorcp_transform_elastix', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    for epoch in range(max_epoch):
        ...
        for batch_idx in tbar:
            ...
            # data augmentation: tumor_cp  #!!! tumor_cp w/ transform Gaussianblur
            ...
            '''
            import torchvision
            torchvision.utils.save_image(input_l,'inputl_forcp.png')
            torchvision.utils.save_image(target_l.unsqueeze(1).float(),'targetl_forcp.png')
            torchvision.utils.save_image(input_l_aug,'input_l_aug_forcp.png')
            torchvision.utils.save_image(target_l_aug.unsqueeze(1).float(),'target_l_aug_forcp.png')
            if epoch>1:
                print(aaa)
            '''
            
            
            volume_batch = torch.cat([input_l,input_l_aug], 0)
            label_batch = torch.cat([target_l,target_l_aug], 0)
            ...
            aug_supervision_1 =  (ce_loss(outputs_1[args.batch_size_label:args.batch_size_label*2], label_batch[args.batch_size_label:args.batch_size_label*2].long()) + dice_loss(
            ...
            model1_loss = labeled_loss_1+aug_supervision_1
            loss = model1_loss
    ...
    logging.info(
        "Total training time: {} ({:.4f}s / epoch)".format(total_training_str, total_training_time / max_epoch))
