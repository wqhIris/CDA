...
from networks.scgm_network import my_net #from network.scgm_network import my_net
...
from config_scgm_loadaugbaseline_1gpu_C import default_config
...
def ini_model(restore=False, restore_from=None):
    if restore:
        ...
        model_l = my_net(modelname='mydeeplabV3P', default_config=default_config) #!!!my_net(modelname='mydeeplabV3P')
        model_r = my_net(modelname='mydeeplabV3P', default_config=default_config) #!!!my_net(modelname='mydeeplabV3P')

    #!!!
    if default_config['Loadselftrain']:
        model_dict_l = model_l.state_dict()
        model_dict_r = model_r.state_dict()
        pretrained_dict = torch.load(default_config['pretrain_file']) 
        pretrained_dict = pretrained_dict.state_dict()
        pretrained_dict_l = {k : v for k, v in pretrained_dict.items() if k in model_dict_l}
        pretrained_dict_r = {k : v for k, v in pretrained_dict.items() if k in model_dict_r}
        model_dict_l.update(pretrained_dict_l) 
        model_l.load_state_dict(model_dict_l)
        print('load {}'.format(default_config['pretrain_file'])) 
        model_dict_r.update(pretrained_dict_r) 
        model_r.load_state_dict(model_dict_r)
        print('load {}'.format(default_config['pretrain_file'])) 
    #!!!
...
def train_one_epoch(model_l, model_r, niters_per_epoch, label_dataloader, unlabel_dataloader_0, unlabel_dataloader_1, optimizer_r, optimizer_l, cross_criterion, epoch):
    ...
    for idx in pbar:
        ...
        '''
        import torchvision
        torchvision.utils.save_image(imgs, 'imgs.png')
        torchvision.utils.save_image(aug_imgs, 'aug_imgs.png')
        torchvision.utils.save_image(mask[:,0,:,:].unsqueeze(1).float(), 'mask.png')
        torchvision.utils.save_image(unsup_imgs_0, 'unsup_imgs_0.png')
        torchvision.utils.save_image(unsup_imgs_1, 'unsup_imgs_1.png')
        torchvision.utils.save_image(aug_unsup_imgs_1, 'aug_unsup_imgs_1.png')
        torchvision.utils.save_image(aug_unsup_imgs_0, 'aug_unsup_imgs_0.png')
        torchvision.utils.save_image(batch_mix_masks.float(), 'batch_mix_masks.png')
        torchvision.utils.save_image(unsup_imgs_mixed, 'unsup_imgs_mixed.png')
        torchvision.utils.save_image(aug_unsup_imgs_mixed, 'aug_unsup_imgs_mixed.png')
        '''
        ...
        '''
        torchvision.utils.save_image(ps_label_1.unsqueeze(1).float(), 'ps_label_1.png')
        torchvision.utils.save_image(ps_label_2.unsqueeze(1).float(), 'ps_label_2.png')
        
        if epoch > 3:
            print(ps_label_1,ps_label_1.size())
            print(ggg)
        '''
...
def train(label_loader, unlabel_loader_0, unlabel_loader_1, test_loader, val_loader, learning_rate, weight_decay, num_epoch, model_path, niters_per_epoch):
    ...
    for epoch in range(num_epoch):
        ...
        '''
        text = {'val/val_dice': val_dice}
        print(text)
        # test
        '''
        wandb.log({'test/test_dice': test_dice})
        '''
        text = {'test/test_dice': test_dice}
        print(text)
        # loss
        '''
        ...
        '''
        text = {'epoch': epoch + 1, 'loss/total_loss': total_loss, 'loss/total_loss_l': total_loss_l,
                  'loss/total_loss_r': total_loss_r, 'loss/total_cps_loss': total_cps_loss,
                   'loss/test_loss': test_loss, 'loss/val_loss': val_loss, 'loss/con_loss': total_con_loss }
        print(text)
        ...
        if val_dice > best_dice:
            ...
            savedir='./tmodel_scgm/default_loadaugbaseline_C/'
            if not os.path.exists(savedir):
                os.makedirs(savedir)
            model_name_l = savedir + 'l_' + model_path
            model_name_r = savedir + 'r_' + model_path
